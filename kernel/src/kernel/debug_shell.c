...
static void cmd_cat(const char *args[], int argc);
...
static void printf(const char *format, ...)
{
    ...
    va_end(argp);
    write(stdout, tmp_buff, strlen(tmp_buff));
...
static void cmd_ls(const char *args[], int argc)
{
    ...
    char *dir = argc == 1 ? cwd : args[1];
    struct file *d = virtfs_open(dir);
    
    if(d) {
        struct dirent ent;
        for(int i = 0; virtfs_readdir(d, i, &ent) == 0; i++) {
            switch(ent.d_type) {
                case DT_DIR:
